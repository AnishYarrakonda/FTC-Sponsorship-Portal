-- 0094_fulfillment_sponsor_org_members.sql
--
-- Fixes: a sponsor invited through a Clerk Organization could not act on a fulfillment.
--
-- `record_fulfillment_transition` resolved the sponsor branch with the PRE-0082 pattern:
--
--     EXISTS (SELECT 1 FROM profiles p
--              WHERE p.id = v_actor AND p.role = 'sponsor' AND p.sponsor_id = v_f.sponsor_id)
--
-- 0082 moved sponsor resolution off the single `profiles.sponsor_id` column onto
-- `sponsor_members` — that column is only ever stamped on the ORIGINAL account holder, so an
-- invited teammate has it NULL forever — and 0089 swept four call sites onto
-- `sponsor_ids_for_profile()`. This one was missed, and 0091 preserved it while fixing the
-- admin branch immediately above it.
--
-- The failure is CLOSED, not open: the branch does not match, the actor falls through to the
-- final ELSE, and the function returns `unauthorized`. Nothing leaks. What it does instead is
-- make the entire fulfillment state machine — "mark payment sent", the sponsor half of the
-- money flow this platform exists to track — inert for every sponsor who joined by
-- invitation rather than original signup. Same population as the sponsor-page lockout.
--
-- `sponsor_ids_for_profile(uuid)` unions the legacy `profiles.sponsor_id` with every
-- `sponsor_members.sponsor_id` for that profile, so this is strictly additive: a legacy
-- single-seat sponsor matches exactly as before. The `role = 'sponsor'` guard is kept —
-- membership alone must not confer the sponsor actor role on a coach or admin profile.
--
-- Body is otherwise copied verbatim from 0091. Apply with `psql -f` (multiple $$ bodies).
