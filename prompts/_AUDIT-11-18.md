...
| **H-09** | P2 | `appeals.ts:414-511` | **OPEN, and NOT introduced here.** A partially applied overturn is unrecoverable: the subject effect is applied before the `appeals` UPDATE, so if that update fails the appeal stays `under_review` while the subject has moved, and every retry dies on the state guard (`.eq('status','declined')`, `.not('denied_at','is',null)`, `outcome='rejected'`). The file header's rule 2 claims the operation can be retried; past that point it cannot. All three branches share this. | **OPEN** — the honest fix is one RPC that writes subject and appeal in a single transaction. Out of scope for a P3 cleanup pass. |
